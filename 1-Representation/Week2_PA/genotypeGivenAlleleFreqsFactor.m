...
genotypeFactor.card = rows(genotypesToAlleles);
...
for i = 1:rows(allelesToGenotypes)
  for j=1:columns(allelesToGenotypes)
    ...
    % Probabilities of FF, ff, and (Ff + fF)
    genotypeFactor.val(genotype_id) = genotypeFactor.val(genotype_id) + (alleleFreqs(i) * alleleFreqs(j));
  endfor
endfor
