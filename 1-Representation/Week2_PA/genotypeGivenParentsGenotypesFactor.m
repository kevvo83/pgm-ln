...
genotypeFactor.card = [rows(genotypesToAlleles) rows(genotypesToAlleles) rows(genotypesToAlleles)];
...
for x = 1:length(assignments)
  child_genotype = assignments(x, :)(1);
  parent1_genotype = assignments(x, :)(2);
  parent2_genotype = assignments(x, :)(3);
  
  child_allele_combo = genotypesToAlleles(child_genotype, :);
  parent_1_allele_combo = genotypesToAlleles(parent1_genotype, :);
  parent_2_allele_combo = genotypesToAlleles(parent2_genotype, :);
  
  for p1_idx = 1:columns(parent_1_allele_combo)
    for p2_idx = 1:columns(parent_2_allele_combo)
      
      % loop over the allele combos of the parents to see if 1 allele from each 
      %   parent produces an allele combo that matches the childs allele combo
      p_c_order_1 = [parent_1_allele_combo(p1_idx) parent_2_allele_combo(p2_idx)];
      p_c_order_2 = [parent_2_allele_combo(p2_idx) parent_1_allele_combo(p1_idx)];
      
      % CHECK is for both orders - for example, Ff and fF are both genotype 2 
      %   in allesToGenotypes - so we need to check both orders when checking
      %   for similarily
      if p_c_order_1 == child_allele_combo || p_c_order_2 == child_allele_combo
        genotypeFactor.val(x) = genotypeFactor.val(x) + 1;
      endif
    endfor
  endfor
  
  % Finally normalize by total number of allele combinations for each set of
  %  parent and child alleles
  normalizing_constant = prod(size(allelesToGenotypes));
  genotypeFactor.val(x) = genotypeFactor.val(x) / normalizing_constant;
